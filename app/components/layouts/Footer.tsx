...
export default function Footer({ onNavigate }: { onNavigate: (path: string) => void }) {
  ...
  const socialCopy =
    'Follow us on social media to stay connected with the latest news, events, and achievements from our school community.'
  ...
  return (
    <footer className='footer text-[#fffae9] relative z-50 overflow-hidden font-sans'>
      {/* Background Image */}
      <div
    ...
    `
      ...
      <div className='max-w-[800px] mx-auto px-4 md:px-20 pt-12 pb-8'>
        <div className='flex flex-wrap justify-center gap-8'>
          {/* Inquire Button */}
          <button
            onClick={() => onNavigate('/contact/inquire')}
            className='group flex flex-col items-center justify-center w-[140px] h-[140px] md:w-[160px] md:h-[160px] rounded-full transition-all duration-300 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-[#FABA1E] backdrop-blur-md btn-footer bg-white/10 hover:bg-black/40 border border-white/20 hover:border-[#FABA1E]'
          >
            <svg
      ...
      <div className='w-full px-4 md:px-10 py-14'>
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-16'>
          ...
          <div className='lg:mr-8'>
            {/* Logo */}
            <div className='mb-6'>
              <img src={logoImage} alt='LHBS - Lac Hong Bilingual School' className='h-16 w-auto' />
            </div>
            ...
            <div className='space-y-4'>
              ...
              <div className='flex items-start gap-3'>
                <MapPin className='w-5 h-5 text-[#fffae9]/50 flex-shrink-0 mt-0.5' aria-hidden='true' />
                <span className=' text-sm text-[#fffae9]/90 whitespace-pre-line'>{addressLine}</span>
              </div>

              {/* Phone */}
              <div className='flex items-center gap-3'>
                <Phone className='w-5 h-5 text-[#fffae9]/50 flex-shrink-0' aria-hidden='true' />
                <a
                  href={`tel:${phone.replace(/\s/g, '')}`}
                  className=' text-sm text-[#fffae9]/90 hover:text-[#FABA1E] transition-colors focus:outline-none focus:ring-2 focus:ring-[#FABA1E]'
                >
                  {phone}
                </a>
              </div>
              ...
              {/* Email */}
              <div className='flex items-center gap-3'>
                <Mail className='w-5 h-5 text-[#fffae9]/50 flex-shrink-0' aria-hidden='true' />
                <a
                  href={`mailto:${email}`}
                  className=' text-sm text-[#fffae9]/90 hover:text-[#FABA1E] transition-colors focus:outline-none focus:ring-2 focus:ring-[#FABA1E]'
                >
          ...
          {/* Column 2: About LHBS */}
          <div>
            <h3 className=' text-xl font-bold text-[#FABA1E] uppercase tracking-wide mb-6'>About LHBS</h3>
              ...
              {aboutLinks.map((link, index) => (
                <li key={index}>
                  <button
                    ...
                    className=' text-sm text-[#fffae9]/90 hover:text-[#FABA1E] hover:underline transition-colors focus:outline-none focus:ring-2 focus:ring-[#FABA1E] text-left'
                  >
                    {link.label}
                  </button>
                </li>
          ...
          <div>
            <h3 className=' text-xl font-bold text-[#FABA1E] uppercase tracking-wide mb-6'>Our Programs</h3>
          ...
          <div>
            <h3 className=' text-xl font-bold text-[#FABA1E] uppercase tracking-wide mb-6'>Connect With Us</h3>

            <p className=' text-sm text-[#fffae9]/90 mb-6 leading-relaxed'>{socialCopy}</p>

            {/* Social Icons Row */}
            <div className='flex items-center gap-4'>
              {socialLinks.map((social, index) => (
                <SocialIcon key={index} platform={social.icon} url={social.url} ariaLabel={social.ariaLabel} />
              ))}
            </div>
          </div>
        </div>
      ...
      <div className='w-full px-4 md:px-10 py-6'>
        <div className='flex flex-col md:flex-row md:items-center md:justify-between gap-4'>
          {/* Copyright */}
          <p className=' text-xs text-[#fffae9]/70'>{copyrightText}</p>
          ...
          <div className='flex items-center gap-6'>
            {legalLinks.map((link, index) => (
              <span key={index} className='flex items-center gap-6'>
                <button
                  onClick={() => onNavigate(link.url)}
                  className=' text-xs text-[#fffae9]/70 hover:text-[#FABA1E] hover:underline transition-colors focus:outline-none focus:ring-2 focus:ring-[#FABA1E]'
                >
                  {link.label}
                </button>
                {index < legalLinks.length - 1 && (
                  <span className='text-[#fffae9]/40' aria-hidden='true'>
                    ·
                  </span>
                )}
              </span>
...
function SocialIcon({ platform, url, ariaLabel }: SocialIconProps) {
  ...
  return (
    <button
      onClick={handleClick}
      className='group focus:outline-none focus:ring-2 focus:ring-[#FABA1E] focus:ring-offset-2'
      ...
      >
        ...
        {platform === 'facebook' && (
          <path
            ...
            className='fill-[#fffae9] transition-colors duration-300 group-hover:fill-[#FABA1E]'
          />
        )}

        {platform === 'instagram' && (
          <g className='fill-[#fffae9] transition-colors duration-300 group-hover:fill-[#FABA1E]'>
            <path d='M20 16.5C17.925 16.5 16.25 18.175 16.25 20.25C16.25 22.325 17.925 24 20 24C22.075 24 23.75 22.325 23.75 20.25C23.75 18.175 22.075 16.5 20 16.5ZM20 22.5C18.75 22.5 17.75 21.5 17.75 20.25C17.75 19 18.75 18 20 18C21.25 18 22.25 19 22.25 20.25C22.25 21.5 21.25 22.5 20 22.5Z' />
            <path d='M23.875 17.375C24.4963 17.375 25 16.8713 25 16.25C25 15.6287 24.4963 15.125 23.875 15.125C23.2537 15.125 22.75 15.6287 22.75 16.25C22.75 16.8713 23.2537 17.375 23.875 17.375Z' />
            <path d='M25.875 12.625C24.625 11.375 23 11 20 11C17 11 15.375 11.375 14.125 12.625C12.875 13.875 12.5 15.5 12.5 18.5V22C12.5 25 12.875 26.625 14.125 27.875C15.375 29.125 17 29.5 20 29.5C23 29.5 24.625 29.125 25.875 27.875C27.125 26.625 27.5 25 27.5 22V18.5C27.5 15.5 27.125 13.875 25.875 12.625ZM26 22C26 24.75 25.75 25.875 24.875 26.75C24 27.625 22.875 27.875 20 27.875C17.125 27.875 16 27.625 15.125 26.75C14.25 25.875 14 24.75 14 22V18.5C14 15.75 14.25 14.625 15.125 13.75C16 12.875 17.125 12.625 20 12.625C22.875 12.625 24 12.875 24.875 13.75C25.75 14.625 26 15.75 26 18.5V22Z' />
          </g>
        )}
        ...
        {platform === 'linkedin' && (
          <g className='fill-[#fffae9] transition-colors duration-300 group-hover:fill-[#FABA1E]'>
            <path d='M15.375 17.5H12.25V27.5H15.375V17.5Z' />
            <path d='M13.8125 12.5C12.8125 12.5 12 13.3125 12 14.3125C12 15.3125 12.8125 16.125 13.8125 16.125C14.8125 16.125 15.625 15.3125 15.625 14.3125C15.625 13.3125 14.8125 12.5 13.8125 12.5Z' />
            <path d='M21.75 17.25C20.125 17.25 19.125 18.125 18.75 18.875V17.5H16V27.5H18.75V22.25C18.75 20.875 19.25 19.75 20.75 19.75C22.25 19.75 22.5 21.125 22.5 22.375V27.5H25.25V21.75C25.25 19.375 24.75 17.25 21.75 17.25Z' />
          </g>
        )}
      </svg>
    </button>
